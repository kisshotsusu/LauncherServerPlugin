...
```
玩家机器                                   服务器 D:\test\CodeBuild\Server (:8710)
─────────────────────                    ─────────────────────────────
Launcher.exe（单文件）                     run_server.py
...
1. **启动服务器**：运行 `Server\启动服务器.bat`，打开 http://127.0.0.1:8710/
...
4. **管理台**：http://127.0.0.1:8710/ 可完成版本管理、开放版本、启动器发布、
   背景资源设置、服务器控制
...
- 纯 Python 标准库，零第三方依赖；管理台含版本管理、云端文件、完整性清单、
  服务器控制、启动器管理
